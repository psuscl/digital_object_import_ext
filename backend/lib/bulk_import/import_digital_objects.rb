class ImportDigitalObjects < BulkImportParser
  ...
  def check_row
    ...
    normalize_publish_column(@row_hash, 'publish')
  ...
  def create_user_defined
    ...
    @row_hash.select {|k, v| k.to_s.match(/^user_defined_/)}.each do |k, v|
      key = k.to_s.sub('user_defined_', '')
      begin
        # should probably not do this for every column
        if key.start_with?('enum')
          user_defined[key] = CvList.new("#{k}", @current_user).value(@row_hash[k])
        else
          user_defined[key] = @row_hash[k]
        end
      rescue Exception => e
        @report.add_errors(I18n.t("bulk_import.error.#{key}", :what => @row_hash[k]))
      end
    end

    u = JSONModel(:user_defined).new(user_defined)
  end
end